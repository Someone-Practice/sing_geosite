...
func download(release *github.RepositoryRelease) ([]byte, error) {
	...
	if geositeAsset == nil {
		if release.Name == nil {
			return nil, E.New("missing geosite asset name")
		}
...
func generate(release *github.RepositoryRelease, ruleSetOutput string) error {
	...
	for code, domains := range domainMap {
		...
		var plainRuleSetSource option.PlainRuleSetCompat
		plainRuleSetSource.Version = 3
		plainRuleSetSource.Options = option.PlainRuleSet{
		...
		jsonPath, _ := filepath.Abs(filepath.Join(ruleSetOutput, "geosite_"+code+".json"))
		var (
			outputRuleSet       *os.File
			outputRuleSetSource *os.File
		)
		...
		encoder.SetIndent("", "  ")
		err = encoder.Encode(plainRuleSetSource)
